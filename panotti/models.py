...
def RaspberryNet(X_shape, nb_classes, nb_layers=4):
    ...
    input_shape = (X_shape[1], X_shape[2], X_shape[3])
    model = Sequential()
    model.add(Conv2D(nb_filters, kernel_size, strides=1, padding='same', input_shape=input_shape, name="Input"))

    model.add(SeparableConv2D(40, (3,3), strides=2, padding='same', activation='relu'))
    model.add(BatchNormalization(axis=-1))
    model.add(SeparableConv2D(80, (5,5), strides=2, padding='same', activation='relu'))
    model.add(BatchNormalization(axis=-1))
    model.add(SeparableConv2D(112, (5,5), strides=2, padding='same', activation='relu'))
    model.add(BatchNormalization(axis=-1))
